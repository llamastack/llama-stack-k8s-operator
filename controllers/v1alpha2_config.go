...
// Any field that modifies the generated config (providers, resources, storage,
// disabled, or a non-default networking port) triggers config generation.
func DetermineConfigSource(spec *v1alpha2.LlamaStackDistributionSpec) llsconfig.ConfigSource {
	...
	if spec.Providers != nil || spec.Resources != nil || spec.Storage != nil || len(spec.Disabled) > 0 {
...
// Returns nil result if the CR does not use v1alpha2 native config features or if the
...
func (r *LlamaStackDistributionReconciler) handleV1Alpha2NativeConfig(
	...
	// Inject secret-backed env vars into the container spec so the Deployment
	// pod template includes them. The generated config.yaml contains
	// ${env.LLSD_<ID>_<FIELD>} placeholders that resolve to these env vars.
	if len(generated.EnvVars) > 0 {
		v1Instance.Spec.Server.ContainerSpec.Env = append(
			v1Instance.Spec.Server.ContainerSpec.Env, generated.EnvVars...,
		)
	}
