...
The v1alpha1 `secretKeyRef` env var pattern is replaced by the `secretRefs` field on each provider. Each entry is a named secret reference (`name` = Secret name, `key` = key within the Secret). The operator injects each as an environment variable at runtime — the secret value never appears in the CR or ConfigMap.
